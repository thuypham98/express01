...
mongoose.connection.once('open', () => {
    app.listen(3000, () => console.log('Server port 3000 uploadTest started!'));
});
...
app.get('/add', (req, res) => res.render('add')); //create screen

//them san pham
app.post('/add', upload.single('image'), (req, res) => { // image: tên của <input type="file" name="profile" />
...
//edit product
app.get('/edit/:id', (req, res) => { //show edit screen
    const { id } = req.params;
    Product.findById(id)
    .then(product => res.render('editProduct', { product }));
});
app.post('/update/:id', upload.single('image'), (req, res) => { // update product
    const { id } = req.params;
